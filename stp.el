...
;; Version: 0.8.6
;; Package-Requires: ((emacs "29.1") (dash "2.19.1") (f "0.21.0") (s "1.12.0") (queue "0.2") (async "1.9.9") (anaphora "1.0.4") (transient "0.9.4") (memoize "1.2.0") (rem "0.7.6"))
...
(defvar stp-transient-args nil)
(defvar stp-transient-args-kwds nil)

(defmacro stp-transient-define-arg-toggle (name key kwd)
  `(progn
     (transient-define-argument ,(intern (format "stp-transient-kwd-args:%s" name)) ()
       :description ,(s-capitalize name)
       :class 'transient-switch
       :key ,key
       :init-value (lambda (obj) (setf (slot-value obj 'value) (plist-get stp-transient-args ,kwd)))
       :argument ,(s-downcase name)
       :if (lambda () (plist-get stp-transient-args ,kwd)))
     (add-to-list 'stp-transient-args-kwds ',(cons (s-downcase name) kwd))))

(stp-transient-define-arg-toggle "commit" "c" :do-commit)
(stp-transient-define-arg-toggle "push" "p" :do-push)
(stp-transient-define-arg-toggle "lock" "l" :do-lock)
(stp-transient-define-arg-toggle "actions" "a" :do-actions)
(stp-transient-define-arg-toggle "audit" "A" :do-audit)
(stp-transient-define-arg-toggle "tag" "t" :do-tag)

(defun stp-transient-kwd-args-do-toggles ()
  (interactive)
  (let ((toggles (transient-args 'stp-command-kwd-args-transient)))
    (dolist (toggle toggles)
      (setf (plist-get stp-transient-args (map-elt stp-transient-args-kwds toggle))
            (not (plist-get stp-transient-args (map-elt stp-transient-args-kwds toggle)))))))

(transient-define-prefix stp-command-kwd-args-transient ()
  ["Options"
   (stp-transient-kwd-args:commit)
   (stp-transient-kwd-args:push)
   (stp-transient-kwd-args:lock)
   (stp-transient-kwd-args:actions)
   (stp-transient-kwd-args:audit)
   (stp-transient-kwd-args:tag)]
  ["Run"
   ("RET" "Run the command" stp-transient-kwd-args-do-toggles)])
...
(cl-defun stp-command-kwd-args (&key (lock t) actions audit tag (do-commit nil do-commit-provided-p) (do-push nil do-push-provided-p) (do-lock nil do-lock-provided-p) (do-actions nil do-actions-provided-p) (do-audit nil do-audit-provided-p) (do-tag nil do-tag-provided-p) (ensure-clean t))
  ...
  (let ((args
    ...
    (when current-prefix-arg
      (unwind-protect
          (progn
            (setq stp-transient-args args)
            (stp-command-kwd-args-transient))
        (setq stp-transient-args nil)))
...
a transient menu before running the command.
