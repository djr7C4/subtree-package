* What is it
Subtree package (STP) is a package manager for Emacs that is designed to provide
maximum control to power users and elisp developers while being highly
configurable and convenient. Packages are installed and managed as git subtrees.
* Demos
** Installing packages
...
- building source (=b=),
- viewing upstream commits in magit (=l=),
- building info (=m=),
- editing remotes (=e=),
- uninstalling (=x=),
...
#+end_src
For more control, you can set =stp-default-controller-args=. It is also possible
to define your own custom policy by defining a subclass of =stp-controller=.
...
You can use =c= to show new commits that have been made upstream since commit
for the installed version. By default, if the update for the package is stabel,
only commits up until the latest stable version are shown. To override this, set
=stp-latest-show-all-new-commits= to =t=.
...
By default, changes to packages are installed without review. To present diffs
to the user before the code is loaded, add

#+begin_src emacs-lisp
(setq stp-audit-changes nil)
#+end_src emacs-lisp

to your configuration. This helps mitigate supply chain attacks such as the
recent one against the kubernetes.el but can be time consuming. By default, =git
reset= is used to restore the previous state when an audit fails.
...
bootstrapped. The simplest way is to clone
=https://github.com/djr7C4/stp-packages= and configure STP to recognize it as
the directory for installed packages.

Alternatively, instead of cloning the =stp-packages= repository you can create
an =stp-packages= directory yourself and manually install STP and its
dependencies as git subtrees (using =git subtree add= with =--squash=) within
=stp-packages/package-source=. To initialize STP's package database, you'll need
to use =M-x stp-repair-all= once you've installed all the dependencies and STP.

With either method, you'll need to copy the following bootstrap code into
=~/.emacs.d/early-init.el=.

#+begin_src emacs-lisp
  (setq stp-source-directory "<path-to-stp-packages>/package-source"
...
STP requires that you install the =atool= CLI utility if you wish to install
archive, ELPA and compressed URL packages. Additionally, to support certain
non-essential features, the =async= and =queue= packages are required. A few
optional features require the =gh= CLI utility.
