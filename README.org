...
- Your configuration is completely reproducible without depending on any
  ...
  cloning it.
** Straight.el
Straight is a functional package manager and relies on code in your init files
to determine which package should be installed. This consists of expressions
such as
...
One downside of this is that you have to populate the :repo field yourself. This
requires browsing github to search for packages to install. By constrast in STP
you would simply enter el-patch in the UI and STP will find the URL of the git
repository for you. It does this by checking the information available from
package archives like ELPA and MELPA. Once the repository has been selected, you
can select any version to install using incremental completion. This enables
installing stable versions of packages using release tags for better stability
instead of the development versions.
