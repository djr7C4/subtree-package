...
will cause the tip of the upstream branch to be chosen. It is also possible to
define your own custom policy of when the user should be prompted by defining a
subclass of =stp-controller=.
...
- It is highly interactive by default and works hard to avoid forcing you to
  leave Emacs during the installation process (for example to find repositories
  and releases on github).
- There are many configuration options possible. By setting a few variables, you
  can reconfigure STP to automatically resolve user prompts whenever possible or
  prompt according to a custom policy.
