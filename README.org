...
- Your configuration is completely reproducible without depending on any
  ...
  cloning it.
** Straight.el
Straight is a functional package manager and relies on code in your
initialization files to determine which package should be installed. This
...
One downside of this is that you have to populate the =:repo= field yourself.
This requires browsing github to search for packages to install. By contrast in
STP you would simply enter el-patch in the UI and STP will find the URL of the
git repository for you. It does this by checking the information available from
