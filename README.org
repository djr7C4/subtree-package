...
After the packages are installed, restart Emacs, set =stp-source-directory= to
the directory that the packages were installed in and run =M-x stp-repair-all=
to initialize the package information database from the git repository.
...
It is recommended to place the bootstraping code in =early-init.el=.
...
to your STP configuration to improve performance. =stp-setup= adds various
variables to =savehist-additional-variables= in order to avoid repeating
