...
that many commits. For ELPA packages, the number in parentheses is the number of
stable versions since the currently installed version. To show the date a
version was committed instead of the amount of time from the installed version,
set =stp-list-annotated-version= to ='timestamp=.
...
#+begin_src emacs-lisp
  ...
  (setq stp-source-directory "emacs-config/package-source/"
        stp-info-file "emacs-config/pkg-info.eld")
