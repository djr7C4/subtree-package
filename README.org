...
bootstrapped. The simplest way to do this is with =package.el=.
...
#+begin_src emacs-lisp
  ...
  (add-to-list 'load-path (expand-file-name "rem" package-user-dir))
  (add-to-list 'load-path (expand-file-name "subtree-package" package-user-dir))
  ,#+end_src emacs-lisp

  You can then manually install STP from within STP. This will also automatically
  install all of the dependencies of STP as STP packages. Finally, remove the
  packages installed with =package.el= from =M-x list-packages=.
  ...
  Once you've installed STP, put the bootstraping code in
