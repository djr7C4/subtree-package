...
=savehist-additional-variables=. Adding =stp-installed-features=,
=stp-versions=, =stp-headers-elisp-file-requirements-cache= and
=stp-headers-elisp-file-feature-cache= to =savehist-additional-variables=
improves performance when checking if the requirements for packages are
satisfied.
